...
namespace cute{
	...
	struct suite_test {
		...
		void operator()(){
		    ...
		    //             CI: had to add std::placeholders in order to be not ambigious if some boost headers are included in sut
		                std::for_each(theSuite.begin(), theSuite.end(), boost_or_tr1::bind(&test::operator(), std::placeholders::_1));
		}
	};
}
